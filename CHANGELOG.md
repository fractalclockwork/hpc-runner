...
- **Monitoring / cancel test solvers** — **`slurm-sleep-60`** (**`sci-slurm-lammps`**, `sbatch` sleep via **[`docker/slurm_sleep/`](docker/slurm_sleep/)**) and **`local-sleep-60`** (**`dev-system`**) for long-enough runs to exercise invocations and cancel.
- **`HARNESS_SOLVER_WALL_SECONDS`** — Optional line printed from inside **`sbatch`** jobs (sleep and LAMMPS templates) and from **`local-sleep-60`**; high-resolution wall time around the workload (GNU **`date +%s.%N`**, **`EPOCHREALTIME`** fallback, **`awk`** delta). The harness prefers it for **`runtime_seconds`** / metrics for **both** SLURM-backed and local runs when present, otherwise refines from **`sacct`** (**`Start`/`End`**, **`Elapsed`**, **`ElapsedRaw`**) when SLURM job ids exist. Implemented in **`src/core/src/harness/slurm_elapsed.py`** after subprocess completion (see **`refine_run_result_runtime_from_slurm`** in **`runner.py`**).
