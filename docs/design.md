...
**Run Jobs:** Offer a select box to choose the tests then allow user to run them all or selected ones with a click, showcase the results on the bottom of the run. (ADD A SENTENCE ABOUT WHAT THIS CONNECTS TO ON BACKEND) If user clicks on results, it will take them to page 3 to explore in further detail the results given. 
...
**Heatmap:** (INSERT MORE INFORMATION HERE I FORGET WHAT THIS IS FOR)
...
Icons: I don't think being an emoji-heavy platform is professional, but we can use gears and other icons for the sidebar for better user interactivity
