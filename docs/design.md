...
**Heatmap:** Used in an example given by William for TACC's test harness, gives more granular information about the test results or performance of a run, say eight different metrics across eight dates. These are shown directly under the performance drift analysis plot and display all defined metrics across the filtered time period for a solver. Another component should allow the comparison of different solvers for shared metrics for a selected time period, i.e. the runtime performance of Gaussian vs Q-Chem vs ACES for the past 15 runs that included all 3 solvers. Another component should handle system solver combinations etc. They provide a 2 dimensional comparison of solver, system or hardware combinations for different time points for a particular shared metric.
