...
**baseline (run)** — A reference run selected per solver for comparison. Only one baseline is active per `solver_name` at a time. Stored in the `runs` table (e.g. via an `is_baseline` flag) and set via the API/UI.
